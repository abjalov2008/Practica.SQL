...
alter table rent_info rename add ;
...
select o.first_name AS owner_name, h.house_type from owners o join houses h    ON o.id = h.owner_id order by length(o.first_name) desc limit 1;
...
select * from houses names off cow owners and come up with customer names('%A')
